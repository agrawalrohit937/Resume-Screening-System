...
// import { useFaceDetection, useSimpleFaceMonitor } from '../hooks/useFaceDetection'
...
function CameraPanel({ videoRef, faceStatus, gazeDir, cheatingData, warningCount, maxWarnings }) {
  const STATUS = {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      ...
      <div style={{ position:'relative', borderRadius:16, overflow:'hidden', background:'#0F172A',
        ...
        {['top-2 left-2','top-2 right-2','bottom-2 left-2','bottom-2 right-2'].map((pos,i) => (
          <div key={i} className={`absolute ${pos} w-5 h-5`}
            style={{
              borderColor: STATUS.color, borderStyle:'solid', borderWidth:0,
        ...
        <div style={{ position:'absolute', top:8, left:8, display:'flex', alignItems:'center', gap:5,
          padding:'3px 8px', borderRadius:20, background:'rgba(0,0,0,0.6)', backdropFilter:'blur(4px)' }}>
          <motion.div animate={{ opacity:[1,0.3,1] }} transition={{ repeat:Infinity, duration:1.5 }}
            style={{ width:6, height:6, borderRadius:'50%', background:'#F43F5E' }}/>
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:'white', fontWeight:600, letterSpacing:'0.08em' }}>REC</span>
        </div>

        {/* Status */}
        <div style={{ position:'absolute', bottom:8, left:8, right:8, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:5, padding:'3px 8px', borderRadius:20,
          ...
          {gazeDir !== 'center' && (
            <div style={{ padding:'3px 8px', borderRadius:20, background:'rgba(245,158,11,0.8)', backdropFilter:'blur(4px)' }}>
              <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:'white', fontWeight:600 }}>
                {gazeDir === 'left' ? '← Looking Left' : gazeDir === 'right' ? '→ Looking Right' : gazeDir === 'up' ? '↑ Looking Up' : '↓ Looking Down'}
              </span>
            </div>
          )}
        </div>
      ...
      {/* Integrity score */}
      <div style={{ padding:'10px 14px', borderRadius:12, background: cheatPct > 50 ? '#FFF1F2' : cheatPct > 20 ? '#FFFBEB' : '#ECFDF5',
        ...
        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
          ...
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:11, fontWeight:700,
            color: cheatPct > 50 ? '#F43F5E' : cheatPct > 20 ? '#F59E0B' : '#10B981' }}>
            {warningCount}/{maxWarnings} warnings
          </span>
        </div>
        <div style={{ height:5, background:'#E2E8F0', borderRadius:100, overflow:'hidden' }}>
          <div style={{ height:'100%', borderRadius:100, transition:'width 0.6s ease',
            width:`${cheatPct}%`,
            background: cheatPct > 50 ? '#F43F5E' : cheatPct > 20 ? '#F59E0B' : '#10B981' }}/>
        </div>
...
export default function LiveInterviewV2() {
  ...
  // In the component body, alongside existing videoRef:
  ...
  // Face detection
  // const { faceStatus, faceCount, gazeDir } = useFaceDetection({
  //   videoRef,
  ...
  onEvent:        handleCheatingEvent,
  active:         session.phase === SESSION_PHASE.ACTIVE,
  faceInterval:   1500,    // MediaPipe check every 1.5s
  objectInterval: 4000,    // COCO-SSD every 4s
  emotionInterval:5000,    // face-api.js every 5s
})
  ...
  useEffect(() => {
    ...
    // ── Auto-submit listener for Time Up ────────────────────────────────────────
  useEffect(() => {
    const handleAutoTrigger = () => {
      console.log("Time is up! Triggering auto-submit...");
      
      // 1. Agar answer khali hai toh default text bhej dein
      const finalAnswer = answerText.trim() || "Time up: No answer provided.";
      
      // 2. Submit call karein
      handleSubmitAnswer({ 
    ...
    // Custom event listen karein jo useInterviewSession se bhej rahe hain
    window.addEventListener('INTERVIEW_TIME_UP', handleAutoTrigger);
    
    return () => window.removeEventListener('INTERVIEW_TIME_UP', handleAutoTrigger);
  }, [answerText, handleSubmitAnswer]);
                ...
                <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:8 }}>
                  {cameraError ? (
                    <>
                      ...
                      <p style={{ fontSize:12 }}>{cameraError}</p>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14, minWidth:0 }}>
          ...
          <div style={{ background:'white', borderRadius:16, border:'1px solid #E2E8F0', padding:12 }}>
            <p style={{ fontFamily:"'Inter',sans-serif", fontSize:11, fontWeight:700,
              color:'#64748B', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:10 }}>
              AI Detection
            </p>
            <DetectionPanel
              ...
              warningCount={session.cheatingData.warning_count}
              maxWarnings={MAX_WARNINGS}
              cheatingScore={session.cheatingData.score}
            />
          </div>
