...
async def link_or_create_user(
    ...
    # If user exists and has a custom upload (profile_picture_public_id), strip profile_picture from updates
    # so we never overwrite a custom upload with a provider picture
    clean_updates = user_updates.copy() if user_updates else None
    if existing_user and existing_user.profile_picture_public_id and clean_updates:
